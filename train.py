...
def main():
    ...
    if args.mode == "2class":
        ...
        num_classes = 2
        result_file = os.path.join("output", "skip_vote_net_2class.csv")
    else:
        print("Using 3-class problem (TDC vs ADHDC vs ADHDI)")
        num_classes = 3
        result_file = os.path.join("output", "skip_vote_net_3class.csv")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(np.arange(len(dataset))), 1):
        ...
        model = SkipVoteNet(num_classes=num_classes, K=7, input_size=116).to(DEVICE)
